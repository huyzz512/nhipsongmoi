...
<body>
    ...
    <div class="main-content">
        <div class="topbar">
            ...
            <a href="index.php?controller=admin&action=banners" style="text-decoration: none; color: #555;"><i class="fas fa-arrow-left"></i> Quay lại</a>
        </div>
        <div class="content-body">
            <div class="form-wrap">
                ...
                <form action="index.php?controller=admin&action=<?= $action_form ?>" method="POST" enctype="multipart/form-data">
                    <div class="form-group">
                        <label>Vị trí đặt Banner (*)</label>
                        <select name="position" class="form-control" required>
                          <option value="home_top" <?= ($b['position'] ?? '') == 'home_top' ? 'selected' : '' ?>>Top Trang Chủ (Ngang dài)</option>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="target_url" class="form-control" value="<?= htmlspecialchars($b['target_url']) ?>" placeholder="Ví dụ: https://vdpr.vn/giai-dau">
                    </div>

                    <div class="form-group">
                        <label>Hình ảnh Banner <?= $action_form == 'create_banner' ? '(*)' : '(Để trống nếu không muốn đổi)' ?></label>
                        <?php if(!empty($b['image_url'])): ?>
                            <div style="margin-bottom: 10px;">
                                <img src="<?= $b['image_url'] ?>" style="max-width: 200px; border: 1px solid #ddd; border-radius: 4px;">
                            </div>
                        <?php endif; ?>
                        <input type="file" name="image" class="form-control" accept="image/*" <?= $action_form == 'create_banner' ? 'required' : '' ?>>
                    </div>
