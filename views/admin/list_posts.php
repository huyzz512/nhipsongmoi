...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', sans-serif; }
        ...
        .btn-action { padding: 6px 12px; border-radius: 4px; color: #fff; text-decoration: none; font-size: 0.85rem; margin-right: 5px; display: inline-block; transition: 0.2s;}
...
<div class="sidebar">
        ...
        <a href="index.php?controller=admin&action=list_posts" class="active"><i class="fas fa-list"></i> Danh sách bài viết</a>
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <form action="index.php?controller=admin&action=update_priority" method="POST" style="margin: 0;">
                                    <input type="hidden" name="id" value="<?= $post['id'] ?>">
                                    <select name="priority" onchange="this.form.submit()" <?= $role !== 'admin' ? 'disabled' : '' ?> style="padding: 6px; border-radius: 4px; border: 1px solid #ccc; font-weight: bold; color: <?= (isset($post['priority']) && $post['priority'] > 0) ? '#b71c1c' : '#555' ?>; outline: none; cursor: pointer;">
                            ...
                            <td>
                                <a href="index.php?controller=post&slug=<?= $post['slug'] ?>" target="_blank" class="btn-action btn-view" title="Xem bài viết"><i class="fas fa-eye"></i></a>
                                
                                <?php if($role === 'admin'): ?>
                                    <?php if($post['status'] == 'pending'): ?>
                                        <a href="index.php?controller=admin&action=approve_post&id=<?= $post['id'] ?>" class="btn-action btn-approve" title="Duyệt xuất bản"><i class="fas fa-check"></i></a>
                                    <?php endif; ?>
                                    <a href="index.php?controller=admin&action=delete_post&id=<?= $post['id'] ?>" class="btn-action btn-delete" title="Xóa bài viết" onclick="return confirm('Bạn có chắc chắn muốn xóa bài viết này không?');"><i class="fas fa-trash"></i></a>
                                <?php endif; ?>
                            </td>
                        </tr>
                        <?php endforeach; else: ?>
